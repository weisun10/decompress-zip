...
var extractors = {
    ...
    store: function (file, destination, zip) {
        ...
        if (file.uncompressedSize === 0) {
            ...
            var input = fs.createReadStream(zip.filename, {start: file._offset, end: file._offset + file.uncompressedSize - 1});
    ...
    deflate: function (file, destination, zip) {
        ...
        .then(function () {
            if (file._maxSize <= zip.chunkSize) {
                ...
                var input = fs.createReadStream(zip.filename, {start: file._offset});
    ...
    copy: function (file, destination, zip, basePath) {
        ...
        .then(function (linkTo) {
            ...
            .then(function (stats) {
                if (stats.isFile()) {
                    type = 'File';
                    var input = fs.createReadStream(path.resolve(parent, linkTo));
